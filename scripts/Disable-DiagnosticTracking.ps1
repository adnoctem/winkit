...
.PARAMETER ExportTemplate
  Export the default diagnostic tracking settings as a JSON config template and exit. Use
  -ExportPath to write it to a file instead of printing to the console.
.PARAMETER ExportState
  Export the current diagnostic tracking settings as reusable JSON config and exit.
...
.EXAMPLE
  PS> ./Disable-DiagnosticTracking.ps1 -ExportTemplate -ExportPath '.\diagnostic-tracking.json'
  Exports the default diagnostic tracking profile.
.EXAMPLE
  PS> ./Disable-DiagnosticTracking.ps1 -ExportState -ExportPath '.\diagnostic-tracking-current.json'
  Exports the current diagnostic tracking settings to a JSON file.
.LINK
...
param (
  ...
  [Parameter(
    Mandatory = $false,
    HelpMessage = 'Export the default diagnostic tracking settings as a JSON config template and exit.'
  )]
  [switch]
  $ExportTemplate,
  ...
  [Parameter(
    Mandatory = $false,
    HelpMessage = 'Export the current diagnostic tracking settings as reusable JSON config and exit.'
  )]
  [switch]
  $ExportState,
